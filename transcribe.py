...
def read_audio(ws, timeout):
    ...
    song = AudioSegment.from_wav("./audio/bipe.wav")
    play(song)
    #call(["python", "/home/pi/newThomasTest/Snowboy/pixels_demo.pyw"])
...
def on_close(ws):
    ...
    transcript = "".join([x['results'][0]['alternatives'][0]['transcript']
                          for x in FINALS])
#    print(transcript)
    arquivo = open("./transcript.json", 'w')
    transcript_json = json.dumps(transcript, indent=2).encode('utf-8')
    print("json" + transcript_json) 
    transcript_json = transcript_json.decode('utf-8')
    arquivo.write(transcript_json)
    arquivo.close()
    return transcript
...
def on_open(ws):
    ...
    data = {
        ...
        "inactivity_timeout": -1, # in order to use this effectively
...
#    conversationCall(transcript)
    ...
    print("call search.py")
    call(["python3", "./search.py"])
    print("sucefully")
